...
public class MarksEditController {
    ...
    public String displayStudent(@ModelAttribute SearchForm searchForm, UpdateForm updateForm, WebRequest request) {
        try {
            ...
            UpdateObject updateObject = ServiceUtils.getUpdateMarksService().findByStudentId(searchForm.getStudentId().trim(), branchName.trim(), searchForm.getYear().trim(), Integer.parseInt(searchForm.getSemister().trim()), searchForm.getPaper().trim(), searchForm.getInternalType().trim());
